...
from db import db
from models import TagModel, LojaModel, ItemModel
from schemas import TagSchema, TagAndItemSchema

blp = Blueprint("Tags", "tags", description="Operations on tags")

@blp.route("/loja/<int:loja_id>/tag")
...
class Tag(MethodView):
    ...
    @blp.alt_response(
        400,
        description="Returned if the tag is assigned to one or more itens. In this case, the tag is not deleted."
    )
    def delete(self, tag_id):
        ...
        abort(
            400,
            message="Could not delete tag. Make sure tag is not associated with any itens, then try again."
        )
